...
func Output(logType int, format string, v ...interface{}) { ///信息
	...
	case LogWarn:
		prefixStr = "[Warn]"
		c = colors.New(colors.Stdout, colors.Yellow, colors.Black)
	case LogError:
		prefixStr = "[Error]"
		c = colors.New(colors.Stdout, colors.Red, colors.Black)
	case LogFatal:
		prefixStr = "[Fatal]"
		c = colors.New(colors.Stdout, colors.Red, colors.White)
	}
