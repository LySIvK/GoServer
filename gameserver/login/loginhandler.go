...
func (self *Msg_PlayerLogin) checkAction(player *PlayerMgr.Player, msg *retmsg.PlayerLogin_Res) bool {
	if self.AccountID <= 0 {
		loger.Error("AccountID is invalid. AccountID: %v", self.AccountID)
		msg.StatusCode = retmsg.RE_INVALID_ACCOUNTID
		return false
	}

	if len(self.LoginKey) <= 0 {
		loger.Error("LoginKey is invalid. LoginKey: %s", self.LoginKey)
		msg.StatusCode = retmsg.RE_INVALID_PASSWORD
		return false
	}
	...
	if res.StatusCode != retmsg.RE_SUCCESS {
		...
		msg.StatusCode = retmsg.RE_NOT_LOGIN
		return false
	}
...
func (self *Msg_PlayerLogin) payAction(player *PlayerMgr.Player, msg *retmsg.PlayerLogin_Res) bool {

	return true
}

func (self *Msg_PlayerLogin) doAction(player *PlayerMgr.Player, msg *retmsg.PlayerLogin_Res) bool {
...
func (self *Msg_PlayerLogin) ProcessAction(player *PlayerMgr.Player) bool {
	//! 创建返回信息
...
func (self *Msg_CreateRole) checkAction(player *PlayerMgr.Player, msg *retmsg.CreateRole_Res) bool {
	if self.AccountID <= 0 {
	...
	if len(self.PlayerName) <= 0 {
		...
		msg.StatusCode = retmsg.RE_INVALID_PLAYERNAME
		return false
	}
	...
	//! 判断玩家是否已创建角色
	info := player.PlayerMgr.GetPlayerInfoFromAccount(self.AccountID)
	if info != nil {
...
func (self *Msg_CreateRole) payAction(player *PlayerMgr.Player, msg *retmsg.CreateRole_Res) bool {
	return true
}

func (self *Msg_CreateRole) doAction(player *PlayerMgr.Player, msg *retmsg.CreateRole_Res) bool {
	//! 创建角色
	info := new(PlayerMgr.PlayerInfo)
	...
	//! 数据库添加一条记录
	player.PlayerMgr.AddPlayerInfoToDB(info)
	return true
}

func (self *Msg_CreateRole) ProcessAction(player *PlayerMgr.Player) bool {
	msg := new(retmsg.CreateRole_Res)
	msg.StatusCode = retmsg.RE_UNKNOW_ERR
	defer player.Send(msg)
